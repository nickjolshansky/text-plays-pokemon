...
def index():
    time.sleep(2)
    first_paragraph = tv_script.strip().split('\n\n')[0]
    words = first_paragraph.split()
    return render_template('index.html', words=words)

@app.route('/highlight-word', methods=['POST'])
def handle_highlighted_word():
    data = request.get_json()
    highlighted_word = data['word']

    input = executeAndReturnInput(highlighted_word)

    return jsonify({'message': input})

def executeAndReturnInput(word):
    print(f"The highlighted word is: {word}")
